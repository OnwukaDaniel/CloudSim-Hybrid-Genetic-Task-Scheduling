...
public class DatacenterBrokerMod extends SimEntity {
	...
	// SORTING CLOUDLETS BY LENGTH
		...
		for (int itr = 0; itr < populationSize; itr++) {
			...
			/*
			 * double mutProb = rangeMin + (rangeMax - rangeMin) * r.nextDouble(); if
			 * (mutProb < 0.5) { int i; i = random.nextInt(populationSize) % populationSize;
			 * ArrayList<Gene> l = new ArrayList<Gene>(); l =
			 * initialPopulation.get(i).getGeneList(); Chromosomes mutchromosome = new
			 * Chromosomes(l); int j; j = random.nextInt(numCloudlets) % numCloudlets; Vm
			 * vm1 = sortedListVm.get(0); mutchromosome.updateGene(j, vm1);
